<navbar class="bg-white container">
  <div class="container px-4 py-4 mx-auto flex items-center justify-between sm:py-4 sm:px-12 shadow-lg rounded-md">
    ...
    <a href="/" class="flex-shrink-0">
      <img class="h-16 hover:scale-110 transition-transform" src="{{ asset('img/logo/vertical-logo.png') }}" alt="">
    </a>

    @if (request()->is('/'))
      <form action="{{ route('product.search') }}" method="GET" class="max-w-xs mx-auto w-full hidden md:block lg:max-w-lg">
        ...
        <div class="relative">
          ...
          <input type="search" name="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Cari Sepatu Terbaru..." />
          <button type="submit" class="text-white absolute end-2.5 bottom-2.5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 transition-colors">Search</button>
        </div>
      </form>
    @endif
